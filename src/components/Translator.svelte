...
<div class="m-3 flex flex-col gap-2 md:flex-row">
  <div class="w-full rounded-md bg-white/3 p-2 ring-white/50 transition-all focus-within:(bg-white/7 ring-1.7) lg:p-4 sm:p-3">
    ...
    <div class="flex flex-row self-end justify-end">
      <button on:click={sync} class="rounded-lg p-2 text-$c-fg ring-1.2 ring-$c-fg ring-inset transition-all not-disabled:(bg-$c-fg text-$c-bg active:scale-95 hover:op-85) disabled:op-40" disabled={!text || loading}>
        <div class="min-h-4.5 min-w-4.5 flex flex-row items-center justify-center gap-1.5">
          {#if loading}
